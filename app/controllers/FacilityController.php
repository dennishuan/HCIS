	...
	public function index()
	{
		...
		if (Request::ajax()){
			if (array_key_exists('search', $input) && $input['search'] === 'true'){
				...
				$facilities =$this->facility->where('name', '<>', 'IMPORT')->search($qs)->select(['id', 'abbrev', 'type', 'name', 'phone', 'address', 'postal_code'])->get()->toJson();

				return $facilities;
			}
			else{
				//Show a list of all the facility
				$facilities = $this->facility->where('name', '<>', 'IMPORT')->select(['id', 'abbrev', 'type', 'name', 'phone', 'address', 'postal_code'])->get()->toJson();
